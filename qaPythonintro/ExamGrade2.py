...
def get_student_level():
    while True:
        try:
            student_level = int(input("\n\t>> What is the students current level?(1,2)\n\t>> "))
            if student_level not in [1, 2]:
                print("\n\t>> your options are 1 or 2 silly!\n")
                continue
            else:
...
def get_exam_mark():
    ...
    while True:
        try:
            exam_mark = int(input("\n\t>> what is the students exam mark?\n\t>> "))
...
def check_exam_grade(student_level, exam_mark):
    """
...
def display_grade(student_level, exam_mark, exam_grade):
    """
...
def run_exam_checker(user_name):
    ...
    while True:
        ...
        exam_grade = check_exam_grade(student_level, exam_mark)
        display_grade(student_level, exam_mark, exam_grade)
